Ext.define('morakot.HKStock', {
	...
	constructor: function(config){
		...
		code = config["code"];
		if(code){
			if(Ext.isString(code)) me.code = [code];
			else if(Ext.isArray(code)) me.code = code;
			else if(Ext.isIterable(code)) me.code = Ext.Array.toArray(code);
			else {
				alert("请输入正确的股票代码");
				return;
			}
