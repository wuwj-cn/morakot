...
function runAppUpgrade(proj) {
    ...
    // v3 app
        ...
        var appPath = proj.getProperty('app.dir'),
            ...
            noAppJs = (proj.getProperty("args.noappjs") + '') === "true",
            appBackupExcludes = [
                ".sencha_backup/**/*"
            ],
            ...
            currFrameworkVerStr = proj.getProperty("framework.version") + '';
        ...
        if(!appFrameworkVerStr || appFrameworkVerStr == "null") {
            appFrameworkVerStr = "2.1.0";
        }

        var appVer = new Version(appVerStr),
            appFrameworkVer = new Version(Version.cleanVersionString(appFrameworkVerStr)),
            currFrameworkVer = new Version(Version.cleanVersionString(currFrameworkVerStr));

        var force = proj.getProperty("args.force") + '',
            sameCmdVer = new Version(proj.getProperty("cmd.version")).compareTo(appVer) === 0,
            sameFrameworkVer = noFramework || currFrameworkVer.compareTo(appFrameworkVer) === 0;

        if(sameCmdVer && sameFrameworkVer) {
            if(force !== "true") {
        ...
        if(appVer.isLessThan("4.0.0.191")) {
            // remove imports from build.xml
            var buildXmlPath = resolvePath(appPath,"build.xml"),
                buildXmlData = FileUtil.readFile(buildXmlPath) + '';
            buildXmlData = buildXmlData.replace(/<import file=\"(.*?)build-cordova(.*?)\"\/>/g,"");
            FileUtil.writeFile(buildXmlPath, buildXmlData);
            // remove xml files from .sencha
            var senchaPath = resolvePath(appPath, ".sencha", "app"),
                senchaDir = new File(senchaPath),
                files = senchaDir.listFiles(),
                f, flen = files.length, file;
            for(f=0;f<flen;f++) {
                file = files[f];
                if(file.getName().startsWith("build-cordova")) {
                    file["delete"]();
                }
            }
        }
