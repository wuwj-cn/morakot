...
function removeOverrideRequireTag(appJsFile) {
    _logger.debug("Removing @packageOverride requirement from app.js");
    var data = readLines(appJsFile),
        linesToRemove = [
            '// DO NOT DELETE - this directive is required for Sencha Cmd packages to work.',
            '//@require @packageOverrides'
        ],
        len = data.length, line, l,
        nlen = linesToRemove.length, nline, n,
        newData = [], skip;
    ...
    for(l = 0; l < len; l++) {
        ...
        skip = false;
        for(n = 0; n < nlen; n++) {
            nline = linesToRemove[n];
            if(line.trim() === nline.trim()) {
                skip = true;
                break;
            }
        }
...
function runAppUpgrade(proj) {
    var //basedir = proj.getProperty("basedir"),
        ...
        noBackup = (proj.getProperty("args.nobackup") + '') === "true",
    ...
    _logger.info("Updating application and workspace from Ext JS " + appFrameworkVerStr +
            " / Cmd " + appVerStr);
    ...
    if (new Version('4.2.0').compareTo(appFrameworkVer) > 0) {
        ...
        var appJsPath = resolvePath(appPath, "app.js");
        if(new File(appJsPath).exists()) {
            removeOverrideRequireTag(appJsPath);
        }
